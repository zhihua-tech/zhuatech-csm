...
@Entity @Table(name="csm_health_signal") public class HealthSignal extends BaseEntity {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    protected HealthSignal(){} /**
                                * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
                                */
public HealthSignal(String code,String name,CustomerPortfolio customerPortfolio,Status status,int oee){this.code=code;this.name=name;this.customerPortfolio=customerPortfolio;this.status=status;this.oee=oee;this.lastHeartbeat=LocalDateTime.now();}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
